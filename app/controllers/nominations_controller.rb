class NominationsController < ApplicationController
  before_action :set_nomination, only: [:show, :edit, :update, :destroy, :fail, :second]
  respond_to :json

  # POST /nominations/fail
  def fail
    if @nomination.active
      @nomination.update_attribute(:active, false)
    end
  end
  ...
  def second
    if @nomination.active
      @nomination.update_attributes(active: false, seconder: current_user)
      @nomination.perform!
    end
  end
  ...
  def index
    @nominations = Nomination.where(active: true).map(&:map)
  ...
  def create
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.json { render action: 'show', status: :created, location: @nomination }
      else
  ...
  private
    ...
    def nomination_params
      params.require(:nomination).permit(:receiver_id, :reason, :action)
    end
end
